...
func (r *WSO2SPReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !controllerutil.ContainsFinalizer(sp, spFinalizer) {
		...
		// Update triggers a new reconcile, so return here
		return ctrl.Result{}, nil
	}

	// 5. Reconcile the application
...
func (r *WSO2SPReconciler) reconcileAdminAccess(ctx context.Context, sp *wso2v1alpha1.WSO2SP, wso2Client *wso2.Client) (ctrl.Result, error) {
	...
	if err := wso2Client.SetAdminGroupMembership(groupID); err != nil {
...
func (r *WSO2SPReconciler) upsertOAuthSecret(ctx context.Context, sp *wso2v1alpha1.WSO2SP, id, secret string) error {
	secretName := sp.Name + "-oauth2"
	...
	sec.StringData["client_id"] = id
	sec.StringData["client_secret"] = secret
...
func (r *WSO2SPReconciler) reconcileApplicationOAuth2(ctx context.Context, sp *wso2v1alpha1.WSO2SP, wso2Client *wso2.Client) (ctrl.Result, error) {
	...
	if existing == nil {
		...
		if err := wso2Client.CreateApplicationOAuth2Config(sp.Spec.Name, desired); err != nil {
			r.Recorder.Eventf(sp, corev1.EventTypeWarning, "CreateOAuth2Failed", "Failed to query WSO2IS: %v", err)
			return r.setPhase(ctx, sp, PhaseFailed, err.Error(), 30*time.Second)
		}
		return ctrl.Result{
	...
	if err := r.upsertOAuthSecret(ctx, sp, existing.OAuthConsumerKey, existing.OAuthConsumerSecret); err != nil {
	...
	if oauth2HasChanged(existing, desired) {
		...
		if err := wso2Client.UpdateApplicationOAuth2Config(sp.Spec.Name, desired); err != nil {
			r.Recorder.Eventf(sp, corev1.EventTypeWarning, "UpdateOAuth2Failed", "Failed to query WSO2IS: %v", err)
			return r.setPhase(ctx, sp, PhaseFailed, err.Error(), 30*time.Second)
		}
		...
		if sp.Spec.OAuth2 != nil {
			...
			if err == nil {
				desired.OAuth2 = &wso2.OAuth2Config{
					OAuthConsumerKey:    string(sec.Data["client_id"]),
					OAuthConsumerSecret: string(sec.Data["client_secret"]),
				}
			}
		}
